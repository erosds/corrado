        ...
        >
          ...
          {idx === 0 && (
            <>
              ...
              >
                {/* DIV RELATIVE AGGIUNTO QUI PER LA CROCETTA */}
                <div className="relative inline-block group">
                  ...
                  {carico.data_ritiro && (
                    <button
                      onClick={(e) => {
                        e.stopPropagation(); // Evita interferenze con il drag
...
export default function Carichi() {
  ...
  const caricaDati = async () => {
    try {
      ...
      const ordiniNonInCarico = ordiniFiltrati.filter(o => !o.carico_id);
  ...
  const handleDragEnd = async (event) => {
    ...
    try {
      if (!target.is_ordine_singolo && target.carico_id) {
        ...
        const ordiniIds = [...source.ordini.map(o => o.id), ...target.ordini.map(o => o.id)];
  ...
  return (
    <div className="p-4 md:p-8 max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">
            {carichiBozza.length} da gestire · {carichiAssegnati.length} pronti al ritiro
          </p>
        </div>
      ...
      {loading ? (
        ...
        <>
          <div className="mb-12">
            <h2 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-4 ml-1">Ordini da pianificare</h2>
          ...
          <div>
            <h2 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-4 ml-1">Ordini Assegnati (In attesa di ritiro)</h2>
            <div className="overflow-x-auto bg-white rounded-2xl border border-slate-200">
              <table className="w-full">
                <thead className="bg-slate-50 border-b border-slate-100 text-left text-xs font-semibold text-slate-500 uppercase">
                  <tr>
                    ...
                    <React.Fragment key={c.id}>
                      {c.ordini.map((o, idx) => (
                        <tr key={o.id} className="hover:bg-slate-50/50">
                          ...
                          {idx === 0 && (
                            <>
                              <td rowSpan={c.ordini.length} className="px-4 py-3 text-center align-middle bg-green-50/20">
                              ...
                              <td rowSpan={c.ordini.length} className="px-4 py-3 text-center bg-green-50/20">
                              ...
                              <td rowSpan={c.ordini.length} className="px-4 py-3 text-center">
                                <div className="flex flex-col items-center gap-1">
